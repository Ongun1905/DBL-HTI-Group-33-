...
app = DjangoDash('GraphVisualisation', external_stylesheets=external_stylesheets)
app.title = "Email Network"


# Adjacency matrix functions
def calculateGraphData():
    """
    Function for formatting the graph data for the adjacency matrix.
    """
    ...
    for edge in graph.edges(data=True):
        edgeList = list(edge)
        edgeDict = without_keys(
            edgeList[2], {'fromEmail', 'fromJobtitle', 'toEmail', 'toJobtitle'})
    ...
    normalizedMatrix = []
    if (len(matrix) > 0):
        normalizedMatrix = np.vectorize(
            logarithmicNormalizing)(matrix, 1, maxMatrixElement)

    # Calculating the sentiment matrix
    sentimentMatrix = []
        ...
        for cIdx, cell in enumerate(row):
            ...
            if (not edge_data):
                ...
                for single_edge_data in edge_data:
                    total_sentiment += edge_data[single_edge_data].get(
                        'sentiment')

                averageEdgeSentiment = total_sentiment / len(edge_data)
                normalizedAverageSentiment = cuberoot(averageEdgeSentiment)
    ...
    matrixCombined = []
    for (defaultRow, normRow, sentRow) in zip(matrix, normalizedMatrix, sentimentMatrix):
        matrixCombinedRow = []
        for (defaultCell, normCell, sentCell) in zip(defaultRow, normRow, sentRow):
            matrixCombinedRow.append({
                "default": defaultCell,
                "norm": normCell,
                "sent": sentCell
            })

        matrixCombined.append(matrixCombinedRow)
...
app.layout = html.Div([
    ...
    dcc.Store(id='session', storage_type='session', data=calculateGraphData())
